import type { IndicatorResponse } from "./technicalAnalysisClient.js";
...
function fallbackCopilot(params: {
  ...
  return {
    summary: `${params.symbol} currently leans ${params.action} with ${params.confidence.toFixed(
      0
    )}% confidence based on rule-driven signals and technical structure.`,
    entryPlan: [
    ...
    confidenceNote: `Fallback copilot used because live LLM response was unavailable. RSI: ${
      params.indicators.rsi_14?.toFixed(2) ?? "n/a"
    }, MACD histogram: ${params.indicators.macd_hist?.toFixed(4) ?? "n/a"}.`
  };
}
...
`.trim();
  ...
  try {
    ...
    if (!text) return fallbackCopilot(params);

    const parsed = JSON.parse(text) as Partial<CopilotInsight>;
    if (
