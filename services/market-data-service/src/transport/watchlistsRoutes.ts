...
export function createWatchlistsRouter({ pool, fallbackUserId }: Deps): Router {
  const router = Router();

  router.get("/", async (req: Request, res: Response) => {
    try {
      const userId = resolveUserId(req, fallbackUserId);
      const watchlists = await pool.query<{ id: string; name: string }>(
        `
        SELECT id, name
        FROM watchlists
        WHERE user_id = $1
      ...
      const items = await pool.query<{ watchlist_id: string; symbol: string; name: string | null }>(
        `
      ...
      const itemsByList = new Map<string, Array<{ symbol: string; name: string }>>();
  ...
  router.post("/:id/items", async (req: Request, res: Response) => {
    try {
      ...
      const owns = await pool.query<{ id: string }>("SELECT id FROM watchlists WHERE id = $1 AND user_id = $2", [watchlistId, userId]);
      if (!owns.rows[0]) {
  ...
  router.delete("/:id/items/:symbol", async (req: Request, res: Response) => {
    try {
      const userId = resolveUserId(req, fallbackUserId);
      const watchlistId = z.string().uuid().parse(req.params.id);
