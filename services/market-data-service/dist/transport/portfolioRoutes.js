...
const upsertHoldingBodySchema = z.object({
    ...
    sellCommissionRate: z.coerce.number().finite().min(0).max(100).default(0)
});
    ...
    router.get("/", async (req, res) => {
        ...
        `, [portfolioId]);
            ...
            const baseHoldings = holdingsResult.rows.map((row) => {
                ...
                const netProfit = estimatedNetProceeds - totalInvested;
                const grossReturnPct = costBasis > 0 ? (grossProfit / costBasis) * 100 : 0;
                const netReturnPct = totalInvested > 0 ? (netProfit / totalInvested) * 100 : 0;
                const breakEvenPrice = quantity > 0 && sellCommissionRate < 100
                    ? totalInvested / (quantity * (1 - sellCommissionRate / 100))
                    : avgCost;
                return {
                    symbol: row.symbol,
