...
function resolveUser(req, fallbackUserId) {
...
function extractThreshold(triggerText, fallback) {
    const match = triggerText.match(/-?\d+(\.\d+)?/);
    const parsed = match ? Number(match[0]) : NaN;
    return Number.isFinite(parsed) ? parsed : fallback;
}
function mapHistoryToIndicatorCandles(items) {
    return items
        .filter((item) => typeof item.close === "number" && Number.isFinite(item.close))
        .map((item, index, rows) => {
...
async function evaluateRule(deps, row, userId) {
    ...
    const volume = toNumber(info.tdyShareVolume) ?? 0;
    let triggered = false;
    let metricValue = lastPrice;
    switch (row.type) {
        case "PRICE_BREAKOUT": {
    ...
    const provider = dbChannel === "EMAIL" ? "EMAIL_ENGINE" : dbChannel === "SMS" ? "SMS_GATEWAY" : "PUSH_CENTER";
...
export function createAlertsRouter({ pool, fallbackUserId, offlineMarketService }) {
    const router = Router();
