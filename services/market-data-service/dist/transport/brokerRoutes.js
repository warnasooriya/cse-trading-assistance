...
async function ensurePaperAccount(pool, userId) {
    const result = await pool.query(`
...
async function getDefaultBrokerAccount(pool, userId, provider) {
    const result = await pool.query(`
...
async function getHolding(db, portfolioId, symbol) {
...
export function createBrokerRouter({ pool, offlineMarketService, portfolioName }) {
    const router = Router();
