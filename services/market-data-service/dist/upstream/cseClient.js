...
const arrayUnknownSchema = z.array(z.unknown());
function sleep(ms) {
...
function parseSockJsMessages(text) {
    if (text === "o" || text === "h")
        return [];
...
function payloadHasRows(payload) {
    if (Array.isArray(payload)) {
        return payload.some((row) => !!row && typeof row === "object" && "symbol" in row && String(row.symbol).trim());
    }
    if (payload && typeof payload === "object") {
        const obj = payload;
        if ("symbol" in obj && String(obj.symbol).trim())
            return true;
        const nestedCandidates = [
            obj.sharePriceList,
            obj.todaySharePrice,
            obj.reqTodaySharePrice,
            obj.items,
            obj.data
        ];
        for (const candidate of nestedCandidates) {
            if (Array.isArray(candidate) && candidate.some((row) => !!row && typeof row === "object" && "symbol" in row && String(row.symbol).trim())) {
                return true;
            }
        }
    }
...
async function fetchSockJsStompJsonOnce({ requestDestination, topics, timeoutMs }) {
    ...
    return await new Promise((resolve, reject) => {
        ...
        socket.on("close", () => {
            if (!settled)
                settleErr(new Error("CSE ws closed before receiving response"));
        });
...
async function parseJsonResponse(endpoint, response) {
    ...
    try {
        ...
        throw new Error(`CSE ${endpoint} returned invalid JSON (content-type: ${contentType}): ${error.message}. Body: ${text
            .slice(0, 200)
            .replaceAll("\n", " ")}`);
    }
}
async function postForm(endpoint, formData, schema) {
...
async function postFormUnknown(endpoint, formData = {}) {
    const body = new URLSearchParams(Object.entries(formData)
...
async function postJsonUnknown(endpoint, payload) {
...
export function createCseClient() {
    return {
        ...
        getTodaySharePriceList: async () => fetchSockJsStompJsonOnce({
        ...
        getTopGainers: async (limit) => {
            const items = await postForm("topGainers", {}, arrayUnknownSchema);
            return limit ? items.slice(0, limit) : items;
        },
        getTopLosers: async (limit) => {
            const items = await postForm("topLooses", {}, arrayUnknownSchema);
            return limit ? items.slice(0, limit) : items;
        },
        getMostActiveTrades: async (limit) => {
            const items = await postForm("mostActiveTrades", {}, arrayUnknownSchema);
            return limit ? items.slice(0, limit) : items;
        },
